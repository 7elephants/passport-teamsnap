...
describe('TeamsnapStrategy', function() {
    ...
    describe('issuing authorization request', function() {
      ...
      describe('that redirects to service provider', function() {
        ...
        it('should save state in session', function() {
          ...
          expect(request.session['oauth2:auth.teamsnap.com'].state).to.have.length(24);
          expect(request.session['oauth2:auth.teamsnap.com'].state).to.equal(u.query.state);
        });
      ...
      describe('that redirects to service provider with other data in session', function() {
        ...
        before(function(done) {
          chai.passport.use(strategy)
            ...
            .req(function(req) {
              request = req;
              req.session = {};
              req.session['oauth2:auth.teamsnap.com'] = {};
              req.session['oauth2:auth.teamsnap.com'].foo = 'bar';
            })
            .authenticate();
        });
        ...
        it('should preserve other data in session', function() {
          expect(request.session['oauth2:auth.teamsnap.com'].foo).to.equal('bar');
        });
    ...
    describe('processing response to authorization request', function() {
      ...
      describe('that was approved', function() {
        ...
        before(function(done) {
          chai.passport.use(strategy)
            ...
            .req(function(req) {
              ...
              req.session = {};
              req.session['oauth2:auth.teamsnap.com'] = {};
              req.session['oauth2:auth.teamsnap.com']['state'] = 'DkbychwKu8kBaJoLE5yeR5NK';
            })
            .authenticate();
        });
        ...
        it('should remove state from session', function() {
          expect(request.session['oauth2:auth.teamsnap.com']).to.be.undefined;
        });
      }); // that was approved
      ...
      describe('that was approved with other data in the session', function() {
        ...
        it('should preserve other data from session', function() {
          expect(request.session['oauth2:auth.teamsnap.com'].state).to.be.undefined;
          expect(request.session['oauth2:auth.teamsnap.com'].foo).to.equal('bar');
        });
  ...
  describe('using default session state store with session key option', function() {
    var strategy = new TeamsnapStrategy({
      ...
      callbackURL: 'https://www.example.net/auth/example/callback',
      state: true,
      sessionKey: 'oauth2:auth.teamsnap.com'
    },
    function(accessToken, refreshToken, profile, done) {
      if (accessToken !== '2YotnFZFEjr1zCsicMWpAA') { return done(new Error('incorrect accessToken argument')); }
      if (refreshToken !== 'tGzv3JOkF0XG5Qx2TlKWIA') { return done(new Error('incorrect refreshToken argument')); }
